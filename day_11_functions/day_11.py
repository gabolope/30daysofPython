...
empty_list = list()
empty_string = ''
print('#3:', is_empty(empty_list))
print('#3:', is_empty(empty_string))

# 4 Write different functions which take lists. They should calculate_mean, calculate_median, calculate_mode, calculate_range, calculate_variance, calculate_std (standard deviation).
nums = [27, 83, 6, 59, 42, 91, 18, 74, 33, 67]

def calculate_mean(lt):
    sum = 0
    for i in nums:
        sum += i
    return sum / len(nums)
...
def calculate_mode(lt):
    ...
    for numero, cantidad in conteo.items():
        if cantidad > max_repeticiones:
            ...
            mode = numero
    return mode
print('#4c: mode', calculate_mode(nums) )

nums = [54, 85, 2, 95, 102]
def calculate_range(lt):
    lt.sort()
    return lt[-1] - lt[0]
print('#4d: range', calculate_range(nums))

nums = [4, 7, 13, 16]
def calculate_variance(lt):
    # primero, calculo la media:
    total = 0
    for i in lt:
        total += i
    mean = total / len(lt)
    # segundo, calculo la suma de los cuadrados de las diferencias respecto a la media:
    sqr_sum = 0
    for i in lt:
        sqr_sum += (i - mean) ** 2
    # tercero, divido la suma de los cuadrados por la cantidad de elementos:
    variance = sqr_sum / len(lt)
    return variance
...
def calculate_std(lt):
    ...
    std = variance ** 0.5
    return std
print('#4f: standard deviation', calculate_std(nums))
...
instruments = ['guitarra', 'bateria', 'bajo', 'piano', 'saxo', 'piano']
def is_unique_1(lt):
    for i in lt:
        if lt.count(i) > 1:
            return False
    return True
print('#2a:', is_unique_1(instruments))

def is_unique_2(lt):
    if len(lt) == len(set(lt)):
        return True
    return False
print('#2b:', is_unique_2(instruments))

# 3 Write a function which checks if all the items of the list are of the same data type.
attributes = [25, 25.5, 95]
def is_same_type(lt):
    types_set = set()
    for i in lt:
    ...
    if len(types_set) > 1:
        return False
    return True
print('#3:', is_same_type(attributes))

# 4 Write a function which check if provided variable is a valid python variable
def is_valid(vrb):
    if vrb.isidentifier() == True: # si empieza con número, no es válido.
        return True
    return False
print('#4:', is_valid('aja*j'))    
...
def most_spoken_languajes(lt, num):
    languages_set = set()
    languages_list = list() 
    languages_dict = dict()
    for country in lt:
    ...
    for language in languages_set: 
        count = languages_list.count(language)
        languages_dict[language] = count
    languages_count = sorted(languages_dict.items(), key = lambda x:x[1], reverse = True)
    languages_dict_sorted = dict(languages_count)
    return (list(languages_dict_sorted.keys())[:num])
print('#5a:', most_spoken_languajes(countries_data, 3))

# Create a function called the most_populated_countries. It should return 10 or 20 most populated countries in descending order.
def most_populated_countries(lt, num):
    population_dict = dict()
    for country in lt: 
        name = country['name']
        population_dict[name] = country['population']
    population_sorted = dict(sorted(population_dict.items(), key = lambda x:x[1], reverse = True))
    population_sorted = dict(population_sorted)
    population_sorted = list(population_sorted.keys())[:num]
    return population_sorted
print('#5b:', most_populated_countries(countries_data, 8))